...
function game() {
    ...
    function drawBoard(row, col) {
        ...
        for(var i=1; i<=row; i++) {
            ...
            for(var j=1; j<=col; j++) {
                ...
                td.addEventListener("mouseover", aux2, false);
				td.addEventListener("mouseout", aux3, false);
            }
    ...
    function gameOver(turn) {
        if(bolas === 0){
            confirm = true;
            if(turn === true){
                document.getElementById('win_msg1').style.display ='block';
				scoreA+=10;
				document.getElementById("player1score").innerHTML = scoreA;
				document.getElementById('win_msg1').onclick = function() {
					document.getElementById('win_msg1').style.display = 'none';
				}
            }
            else {
                document.getElementById('win_msg2').style.display='block';
				scoreB+=10;
				document.getElementById("computerscore").innerHTML = scoreB;
				document.getElementById('win_msg2').onclick = function() {
					document.getElementById('win_msg2').style.display = 'none';
				}
            }
            run.addEventListener("click", start, false);
            resetBoard();
    ...
    function changeColor(id) {
		...
		change.style.backgroundColor = "#B8860B";
		var n = id.replace('dot', '');
		while((n-10)>=11) {
			var newid = 'dot'+(n-10);
			document.getElementById(newid).style.backgroundColor = "#B8860B";
			n=n-10;
		}
    }
    
    function revert(id) {
		var revert = document.getElementById(id);
		revert.style.backgroundColor = "#FCD16B";
		var n = id.replace('dot', '');
		while((n-10)>=11) {
			var newid = 'dot'+(n-10);
			document.getElementById(newid).style.backgroundColor = "#FCD16B";
			n=n-10;
		}
	}
